...
export default function GeoView() {  const [organizations, setOrganizations] = useState<Organization[]>([]);
  ...
  useEffect(() => {
    const loadFilteredData = async () => {
      try {
        ...
        setStatsRefreshCount(prev => prev + 1);
        setLoading(false);
      } catch (err) {
  ...
  const mapPoints = [
    ...(filters.type === 'organizations' || filters.type === 'all' ? organizations.map(org => ({
      id: org.id,
      name: org.name,
    ...
    ...(filters.type === 'services' || filters.type === 'all' || filters.type === 'nearby' ? services.map(service => ({
      id: service.id,
      name: service.name,
